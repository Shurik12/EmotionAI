...
FileProcessor::FileProcessor(std::shared_ptr<DragonflyManager> dragonfly_manager,
    ...
    : dragonfly_manager_(std::move(dragonfly_manager)), file_storage_(std::move(file_storage))
{
    try
    {
        initialize_models();
...
void FileProcessor::cleanup_file(const std::string &filepath)
{
    try
    {
        // Check if this is a storage path or local path
        if (filepath.find("uploads/") == 0 || filepath.find("results/") == 0)
        {
            ...
            if (file_storage_->fileExists(filepath))
            {
                ...
                LOG_INFO("Cleaned up file from storage: {}", filepath);
            }
        }
        else
        {
            // This is a local filesystem path (legacy)
            if (fs::exists(filepath))
            {
                fs::remove(filepath);
                LOG_INFO("Cleaned up local file: {}", filepath);
            }
        }
    }
    catch (const std::exception &e)
